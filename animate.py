...
DEMO_BOARD = {
    "title": "SCIM Connect — user provisioning",
    "actors": [
    ...
    "scenes": [
        {"kind": "message", "from": "idp", "to": "scim", "label": "POST /Users",
         "detail": "The source IdP sends a SCIM user resource when a new employee is hired."},
        {"kind": "process", "at": "scim", "label": "Map attributes",
         "detail": "SCIM Connect maps userName, emails and externalId onto the target schema."},
        {"kind": "message", "from": "scim", "to": "target", "label": "POST /Users",
         "detail": "The transformed resource is forwarded to the downstream application."},
        {"kind": "message", "from": "target", "to": "scim", "label": "201 Created",
         "status": "success",
         "detail": "The target returns the created resource with its own id."},
        {"kind": "result", "at": "target", "label": "User provisioned", "status": "success",
         "detail": "The account now exists downstream and the id mapping is stored."},
    ],
}
